...
namespace NaughtyAttributes.Editor
{
    ...
    public class NaughtyInspector : UnityEditor.Editor
    {
        ...
        {
            var d = new Dictionary<string, List<MethodInfo>>();
            foreach(var method in _methods) {
                ButtonAttribute ba = (ButtonAttribute)method.GetCustomAttributes(typeof(ButtonAttribute), true)[0];
                if (!d.ContainsKey(ba.Group)) {
                    d[ba.Group] = new List<MethodInfo>();
                }
                d[ba.Group].Add(method);
            }
            ...
            if (_methods.Any())
            {
                ...
                foreach(var group in d.Keys) {
                    if (group != "") {
                        EditorGUILayout.LabelField(group, EditorStyles.boldLabel);
                    }
                    foreach(var method in d[group]) {
